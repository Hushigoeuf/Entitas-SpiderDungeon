...
{
    public sealed class UpdateRateFlightSystem : IExecuteSystem
    {
        private readonly Contexts _contexts;

        public UpdateRateFlightSystem(Contexts contexts)
        {
            _contexts = contexts;
        }
        ...
        private void ExecuteEntity(FlightEntity entity)
        {
            if (!entity.isEnabled)
            {
                if (entity.hasRate && entity.rate.Value != 0) entity.ReplaceRate(0);
                return;
            }
